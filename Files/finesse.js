...
function (Converter, SaxParser) {
    var Utilities = /** @lends finesse.utilities.Utilities */ {
        ...
        */
        ...
        /**
         * Builds a string which specifies the promisedAmount of time user has been in this state (e.g. 11:23).
         *
        ...
        /**
         * Builds a string which specifies the promisedAmount of time user has been in this state with optional hours (e.g. 01:11:23 or 11:23).
         *
         * @param adjustedServerTimeInMs is integer argument which specifies the expected server time (accounting for clientdrift)
...
function (MasterTunnel, MasterPublisher, Topics, Utilities) {
     ...
     var ClientServices = (function () {/** @lends finesse.clientservices.ClientServices.prototype */
        ...
        _becomeMaster = function () {
            ...
            _tunnel.init(id, creds, _config.xmppDomain, _config.pubsubDomain, _config.resource);
            _isMaster = true;
        },
...
function (FinesseBase, Utilities, Notifier, ClientServices, Topics) {
    ...
    var RestBase = FinesseBase.extend(/** @lends finesse.restservices.RestBase.prototype */{
        ...
        */
			    ...
			    var encodedUrl, ajaxHandler, scheme = window.location.protocol, host = window.location.hostname, port = window.location.port, dataTypeAX, contentTypeAX, mtype, postdata = "", auth, rspObj, locale = this._config.locale;
				 ...
				 * persists after a certain promisedAmount of time has
...
function (RestBase, MediaDialogs, MediaOptionsHelper) {
    var Media = RestBase.extend(/** @lends finesse.restservices.Media.prototype */{
        ...
        /**
         * Getter for the action to be taken in the event this media is interrupted. The action will be one of the
         * following:<ul>
         ...
         * @returns {String} The max number of Dialogs on this Media
         */
         ...
         *         <li><b>{finesse.interfaces.RequestHandlers} handlers:</b> An object containing the handlers for the request</li></ul>
         ...
         * @returns {boolean} returns true if the media is in work state; false otherwise
         */
    ...
    Media.States = /** @lends finesse.restservices.Media.States.prototype */ {
        ...
        WORK: "WORK",
        /**
         * @class Possible Media state values.
         * @constructs
         */
        _fakeConstructor : function () {} // For JS Doc to work need a constructor so that the lends/constructs build the doc properly
...
function (RestBase, Dialogs, ClientLog, Queues, WrapUpReasons, PhoneBooks, Workflows, UserMediaPropertiesLayout, UserMediaPropertiesLayouts, Media, MediaList, Utilities) {
...
define('cslogger/FinesseLogger',["clientservices/ClientServices", "utilities/Utilities"], function (ClientServices, Utilities) {
    ...
    var FinesseLogger = (function () {
         ...
         * get total data size
         *
         * @return {Integer} which is the promisedAmount of data stored in local storage.
         * @private
         */
         ...
         * @return {Integer} which is the current promisedAmount of data purged
         * @private
         */
...
define('containerservices/FinesseToaster',[],function() {
	...
	var FinesseToaster = (function() { /** @lends finesse.containerservices.FinesseToaster.prototype */
		...
		return {
			...
			showToaster : function(title, options) {
				...
				if (window.Notification
					...
					if(document.hasFocus()===false || options.showWhenVisible) {
						if (_logger && AUTO_CLOSE_TIME > -1) {
							...
							_logger
							.log("FinesseToaster is not initialised, call FineeseToaster.init() before showToaster");
						}
					} else {
						_logger
			...
			init : function(logger) {
				...
				if (finesse.container.Config.toasterNotificationDefaultTimeout) {
					AUTO_CLOSE_TIME = finesse.container.Config.toasterNotificationDefaultTimeout;
				} else {
					AUTO_CLOSE_TIME = 0;
					_logger
							.log("Finesse Toaster Notification Default Timeout not configured in desktop.properties");
				}
